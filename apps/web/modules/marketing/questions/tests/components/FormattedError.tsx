...
export const FormattedError: React.FC<Props> = ({ error, path }) => {
	// containerClassName, test-error
	return (
		<div
			className="p-2 whitespace-pre-wrap text-hover"
			// biome-ignore lint/security/noDangerouslySetInnerHtml: sandpack controlled content
...
const formatDiffMessage = (error: TestError, path: string): string => {
	...
	if (error.matcherResult) {
		finalMessage = `<span>${escapeHtml(error.message ?? "")
			.replace(/(expected)/m, `<span class="text-success">$1</span>`)
			.replace(/(received)/m, `<span class="text-destructive">$1</span>`)
			.replace(/(Difference:)/m, "<span>$1</span>")
			.replace(
				/(Expected:)(.*)/m,
				`<span>$1</span><span class="text-success">$2</span>`,
			)
			.replace(
				/(Received:)(.*)/m,
				`<span>$1</span><span class="text-destructive">$2</span>`,
			)
			.replace(/^(-.*)/gm, `<span class="text-destructive">$1</span>`)
			.replace(
				/^(\+.*)/gm,
		...
		_originalScriptCode
			...
			.forEach((code) => {
				...
				const content = escapeHtml(code.content)
					.replace(
						/(describe|test|it)(\()(&#039;|&quot;|`)(.*)(&#039;|&quot;|`)/m,
						`<span>$1$2$3</span><span class="font-semibold">$4</span><span>$5</span>`,
					)
					.replace(
						/(expect\()(.*)(\)\..*)(to[\w\d]*)(\()(.*)(\))/m,
						`<span>$1</span><span class="text-destructive">$2</span><span>$3</span><span style="text-decoration: underline; font-weight: 900">$4</span><span>$5</span><span class="text-success">$6</span><span>$7</span>`,
					);
